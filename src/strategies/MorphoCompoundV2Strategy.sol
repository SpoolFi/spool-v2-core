...
contract MorphoCompoundV2Strategy is Strategy {
    ...
    function _getYieldPercentage(int256) internal override returns (int256 baseYieldPercentage) {
        _updateTempYieldPercentage();
        baseYieldPercentage = _tempYield;
        _tempYield = 0;
    }

    function _updateTempYieldPercentage() private {
        uint256 totalBalance = _getTotalBalance();

        if (totalBalance != _lastBalance && _lastBalance > 0) {
            _tempYield += _calculateYieldPercentage(_lastBalance, totalBalance);

            _lastBalance = totalBalance;
        }
    }
