...
/// @notice Used when Gearbox V2 mint returns an error code.
error BadGearboxV3Deposit();

/// @notice Used when Gearbox V2 redeem returns an error code.
error BadGearboxV3Withdrawal();
...
contract GearboxV3Strategy is Strategy {
    ...
    /// @notice COMP token
    /// @dev Reward token when participating in the Gearbox protocol.
    IERC20 public immutable comp;

    /// @notice Gearbox market
        ...
        returns (int256 compoundedYieldPercentage)
    {
        if (swapInfo.length > 0) {
            uint256 compBalance = _getGearboxReward();
            ...
            if (compBalance > 0) {
                ...
                if (swappedAmount > 0) {
                    ...
                    _depositToGearboxProtocol(IERC20(tokens[0]), swappedAmount);
    ...
    function _depositToProtocol(address[] calldata tokens, uint256[] memory amounts, uint256[] calldata)
        ...
        override
    {
        _depositToGearboxProtocol(IERC20(tokens[0]), amounts[0]);
    }

    function _depositToGearboxProtocol(IERC20 token, uint256 amount) private {
        if (amount > 0) {
            _resetAndApprove(token, address(cToken), amount);

            if (cToken.mint(amount) > 0) {
                revert BadGearboxV3Deposit();
            }
        }
    }

    /**
     * @notice Withdraw lp tokens from the Gearbox market
     */
