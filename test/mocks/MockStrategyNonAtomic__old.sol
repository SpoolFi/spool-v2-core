...
import "../../src/strategies/StrategyNonAtomic__old.sol";
...
contract MockStrategyNonAtomic is StrategyNonAtomic {
    ...
    constructor(
        ...
        protocol = new MockProtocolNonAtomic(tokens[0], atomicityClassification_);
    }
    ...
    function depositFeeShares() external view returns (uint256) {
        return _depositFeeShares;
    }

    function userDepositShares() external view returns (uint256) {
        return balanceOf(address(this)) - _withdrawalFeeShares - _depositFeeShares;
    }
...
contract MockProtocolNonAtomic {
    ...
    constructor(address underlying_, uint256 atomicityClassification_) {
    ...
    function invest(uint256 amountUnderlying) external returns (uint256 amountShares, bool finished) {
        ...
        if (atomicityClassification == 1 || atomicityClassification == 3) {
    ...
    function divest(uint256 amountShares) external returns (uint256 amountUnderlying, bool finished) {
        ...
        if (atomicityClassification == 2 || atomicityClassification == 3) {
    ...
    function _divestInternal(uint256 amountShares, address beneficiary) private returns (uint256 amountUnderlying) {
        amountUnderlying = totalUnderlying * amountShares / totalShares;

        totalUnderlying -= amountUnderlying;
        totalShares -= amountShares;

        IERC20(underlying).safeTransfer(beneficiary, amountUnderlying);
    }
}
