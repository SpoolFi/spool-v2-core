...
contract EthenaStrategyUsdeTest is TestFixture, ForkTestFixture {
    ...
    uint256 toDeposit = 100000 * 10 ** 18;
    // ****************************************************
    ...
    function setUp() public {
        ...
        assetGroup = Arrays.toArray(address(USDe));
        ...
        deal(address(USDe), address(ethenaStrategy), toDeposit, true);
    }

    function test_depositToProtocol() public {
        assertEq(USDe.balanceOf(address(ethenaStrategy)), toDeposit);
        ...
        assertEq(USDe.balanceOf(address(ethenaStrategy)), 0);

        uint256 toMint = sUSDe.previewDeposit(toDeposit);
        ...
        returns (uint256[] memory slippages)
    {
        ...
        slippages = new uint256[](1 + 1 + encodedSwapCallData.length);
    ...
    function test_redeemFromProtocolWithSwap() public {
        ...
        uint256 underlyingBefore = USDe.balanceOf(address(ethenaStrategy));
        uint256 sharesBefore = sUSDe.balanceOf(address(ethenaStrategy));

        MockExchange exchange = new MockExchange(sUSDe, USDe, priceFeedManager);

        deal(address(USDe), address(exchange), 1_000_000 * 10 ** IERC20Metadata(address(USDe)).decimals(), false);
        deal(address(sUSDe), address(exchange), 1_000_000 * 10 ** IERC20Metadata(address(sUSDe)).decimals(), false);
        ...
        vm.recordLogs();
        ethenaStrategy.exposed_redeemFromProtocol(assetGroup, withdrawnShares, slippages);
        ...
        ethenaStrategy.exposed_redeemFromProtocol(
            assetGroup, withdrawnShares, buildSlippages(exchange, entries[0].data)
        );

        uint256 underlyingAfter = USDe.balanceOf(address(ethenaStrategy));
        uint256 sharesAfter = sUSDe.balanceOf(address(ethenaStrategy));

        // since we exchange 1 to 1 there is strict equality
        assertEq(underlyingAfter - underlyingBefore, sharesBefore - sharesAfter, "1");
    ...
    function test_redeemFromProtocol() public {
        ...
        uint256 shouldBeRatio = mintedShares * 10 ** 18 / (mintedShares - withdrawnShares);
        assertApproxEqAbs(sharesBefore * 10 ** 18 / sharesAfter, shouldBeRatio, 1, "3");
    }
    ...
    function test_emergencyWithdrawImplWithSwap() public {
        ...
        // act
        ethenaStrategy.exposed_emergencyWithdrawImpl(
            buildSlippages(exchange, entries[0].data), emergencyWithdrawalRecipient
        );

        // assert
        uint256 USDeBalanceAfter = sUSDe.previewRedeem(sUSDe.balanceOf(address(ethenaStrategy)));
        uint256 balanceOfEmergencyWithdrawalRecipient = USDe.balanceOf(emergencyWithdrawalRecipient);

        uint256 sUSDeBalanceOfStrategy = sUSDe.balanceOf(address(ethenaStrategy));
        uint256 USDeBalanceOfStrategy = USDe.balanceOf(address(ethenaStrategy));
        uint256 underlyingBalanceOfStrategy = USDe.balanceOf(address(ethenaStrategy));

        // 1% slippage
        assertApproxEqAbs(USDeBalanceBefore - USDeBalanceAfter, toDeposit, toDeposit / 100, "1");
        // 6% slippage
    ...
    function test_emergencyWithdrawImpl() public {
        ...
        // 0.25% slippage
        assertApproxEqAbs(USDeBalanceBefore - USDeBalanceAfter, toDeposit, toDeposit / 400, "1");
        // 0.25% slippage
    ...
    function test_getUsdWorth() public {
        ...
        uint256 expectedUsdWorth = priceFeedManager.assetToUsd(address(USDe), toDeposit);
    ...
    function test_compound() public {
        ...
        swapper.updateExchangeAllowlist(Arrays.toArray(address(ENA_USDe_Exchange)), Arrays.toArray(true));
